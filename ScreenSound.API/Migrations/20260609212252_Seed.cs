    ...
    public partial class Seed : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                ...
                INNER JOIN Generos G ON G.Nome = 'Rock'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Everlong', 'The Pretender', 'Learn to Fly');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Queen'
                INNER JOIN Generos G ON G.Nome = 'Rock'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Bohemian Rhapsody', 'Don''t Stop Me Now', 'We Will Rock You');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Led Zeppelin'
                INNER JOIN Generos G ON G.Nome = 'Rock'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Stairway to Heaven', 'Whole Lotta Love', 'Black Dog');
            ...
            migrationBuilder.Sql(@"
                ...
                INNER JOIN Generos G ON G.Nome = 'Pop'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Blank Space', 'Shake It Off', 'Love Story');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Michael Jackson'
                INNER JOIN Generos G ON G.Nome = 'Pop'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Billie Jean', 'Thriller', 'Beat It');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Madonna'
                INNER JOIN Generos G ON G.Nome = 'Pop'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Like a Virgin', 'Material Girl', 'Vogue');
            ");
            ...
            migrationBuilder.Sql(@"
                ...
                INNER JOIN Generos G ON G.Nome = 'MPB'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Oceano', 'Flor de Lis', 'Samurai');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Gilberto Gil'
                INNER JOIN Generos G ON G.Nome = 'MPB'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('Andar com Fé', 'Drão', 'Expresso 2222');

                UPDATE Musicas M
                INNER JOIN Artistas A ON A.Nome = 'Caetano Veloso'
                INNER JOIN Generos G ON G.Nome = 'MPB'
                SET M.ArtistaId = A.Id, M.GeneroId = G.Id
                WHERE M.Nome IN ('O Leãozinho', 'Sampa', 'Cajuína');
            ");
        }
